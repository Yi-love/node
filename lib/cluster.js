...
cluster.isWorker = ('NODE_UNIQUE_ID' in process.env);//是否是工作进程
...
function masterInit() {
  ...
  /**
   * [createWorkerProcess 创建工作进程]
   * @param  {[type]} id  [表示进程id]
   * @param  {[type]} env [需要挂载到process.env对象上的参数对象]
   * @return {[type]}     [子进程]
   */
  function createWorkerProcess(id, env) {
    ...
    workerEnv.NODE_UNIQUE_ID = '' + id; //表示为子进程
