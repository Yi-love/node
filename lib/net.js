...
function Socket(options) {
  ...
  stream.Duplex.call(this, options);//全双工

  if (options.handle) {
    this._handle = options.handle; // private
  } else if (options.fd !== undefined) {
    this._handle = createHandle(options.fd);//根据文件描述符创建句柄
...
/**
 * [read read函数是负责从fd中读取内容.当读成功时，read返回实际所读的字节数，
...
/**
 * [connect 调用connect()发出连接请求]
 * @param  {[type]} self         [socket]
 * @param  {[type]} address      [地址]
 * @param  {[type]} port         [端口]
 * @param  {[type]} addressType  [地址类型]
 * @param  {[type]} localAddress [本地地址]
 * @param  {[type]} localPort    [本地端口]
 * @return {[type]}              [description]
 */
function connect(self, address, port, addressType, localAddress, localPort) {
  ...
  if (localAddress || localPort) {//本地地址
    var bind;
    ...
    err = bind(localAddress, localPort);//绑定地址

    if (err) {//出错
      const ex = exceptionWithHostPort(err, 'bind', localAddress, localPort);//报错
      self._destroy(ex);
      return;
    }
  }
  ...
  if (addressType === 6 || addressType === 4) {
    ...
    if (addressType === 4)
      err = self._handle.connect(req, address, port);//连接
    else
  ...
  if (err) {
    ...
    //错误上报
    const ex = exceptionWithHostPort(err, 'connect', address, port, details);//报错
    self._destroy(ex);
  }
}
...
/**
 * [unref 用户可以对激活的handle解引用来提早结束循环]
 * @return {[type]} [description]
 */
 ...
 * @param {[type]} connectionListener [连接监听函数]
 */
function Server(options, connectionListener) {
  ...
  //只传回调函数 connectionListener
  if (typeof options === 'function') {
    connectionListener = options;
    options = {};
    this.on('connection', connectionListener);
  } else if (options == null || typeof options === 'object') {//不传参数或传
    options = options || {};
 ...
 * @param  {[type]} handle  [类型句柄]
 * @param  {[type]} backlog [description]
 * @return {[type]}         [description]
 */
...
/**
 * [_listen2 创建底层监听，赋值给server对象]
 * @param  {[type]} address     [IP]
 * @param  {[type]} port        [端口]
 * @param  {[type]} addressType [ip簇]
 * @param  {[type]} backlog     [backlog TCP连接已完成队列的大小]
 * @param  {[type]} fd          [文件描述符]
 * @return {[type]}             [description]
 */
Server.prototype._listen2 = function(address, port, addressType, backlog, fd) {
  ...
  if (this._handle) {
    ...
    if (!address && typeof fd !== 'number') {
      rval = createServerHandle('::', port, 6, fd);//创建服务器句柄
    ...
    if (rval === null)//一半情况
      rval = createServerHandle(address, port, addressType, fd);//创建服务器句柄

    if (typeof rval === 'number') {//出错了
      var error = exceptionWithHostPort(rval, 'listen', address, port);
      process.nextTick(emitErrorNT, this, error);
      return;
    }
    this._handle = rval;//真正的server消息传输对象
  }

  this._handle.onconnection = onconnection;
  this._handle.owner = this;

  var err = _listen(this._handle, backlog);//执行c++模块，打通服务
 ...
 * @param  {[type]} addressType [description]
 * @param  {[type]} backlog     [description]
 * @param  {[type]} fd          [description]
 * @param  {[type]} exclusive   [description]
 * @return {[type]}             [description]
 */
 ...
 * 这个函数是给c++调用的
 * @param  {[type]} err          [description]
 * @param  {[type]} clientHandle [其值是由内核自动生成的一个全新的描述字，代表与返回客户的TCP连接。]
 * @return {[type]}              [description]
 */
function onconnection(err, clientHandle) {
  ...
  //触发connection函数,也就是我们createServer出入的函数
...
/**
 * [ref 引用给定的handle, 引用时幂等的
 * 也就是说,如果handle已经有引用了,再次调用该函数没有任何效果]
 * @return {[type]} [description]
 */
